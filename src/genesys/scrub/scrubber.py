...
high-entropy shapes — git SHAs (provenance), Genesys deterministic IDs, and the
...
# git SHA: 7..40 lowercase hex. Genesys deterministic IDs and tombstone hashes are
# matched by their own shapes below.
_GIT_SHA_RE = re.compile(r"\b[0-9a-f]{7,40}\b")

# Genesys deterministic episode ID: day + sequence (DR-24 "named by the deterministic
...
_GENESYS_ID_RE = re.compile(r"\bgen-\d{8}-\d{3,}\b")
...
def _is_allowlisted(token: str) -> bool:
    ...
    Exempts git SHAs, Genesys deterministic IDs, and tombstone hash digests. Only
    ...
    """
    return bool(
        _GENESYS_ID_RE.fullmatch(token)
...
def scrub_text(text: str) -> ScrubResult:
    ...
    def _entropy_repl(m: re.Match) -> str:
        ...
        if len(token) < _ENTROPY_MIN_LEN:
            return token
        # R3 allowlist: never flag git SHAs / Genesys IDs / tombstone hashes.
