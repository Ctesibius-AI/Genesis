...
<title>Genesys · QA console</title>
<style>
  :root{
...
function cards(list){
  ...
  $("#cards").innerHTML = list.map(c => {
    ...
    const detail = (c.actions||[]).length ? (c.actions).map(a => `<div class="drow">
      ...
      : `<div class="dr" style="font-family:var(--display);font-style:italic;color:var(--faint)">Not drained yet — no supervision events. Run <code>genesys-worker once</code>.</div>`;
