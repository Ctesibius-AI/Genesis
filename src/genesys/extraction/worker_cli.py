...
Entry point: genesys-worker (see pyproject.toml [project.scripts]).
Subcommands:
...
it during the offline test suite. The offline suite uses genesys-drain (fixtures-only).
"""
...
_DEFAULT_DATA_ROOT = Path.home() / ".genesys" / "data"
...
def _cmd_once(args: argparse.Namespace) -> int:
    ...
    from genesys.extraction.live import run_once  # noqa: PLC0415
...
def _cmd_serve(args: argparse.Namespace) -> int:
    ...
    from genesys.extraction.live import run_forever  # noqa: PLC0415

    data_root = Path(args.data_root)
    interval = float(args.interval)
    print(f"[genesys-worker] starting serve loop (data_root={data_root}, interval={interval}s)",
          flush=True)
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for genesys-worker CLI (spec §4.14)."""
    ...
    parser.add_argument(
        ...
        help="Path to the Genesys data directory (default: ~/.genesys/data).",
    )
