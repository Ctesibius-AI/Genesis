...
No new dependencies; stdlib + internal genesys modules only.
"""
...
from genesys.hooks.adapter import _read_jsonl
...
def save_moment(
    ...
    *,
    ...
    Args:
        data_root: Genesys data root (ledger + WAL live here).
...
def main() -> None:
    ...
    parser.add_argument("--data-root", default="~/.genesys/data",
                        help="Genesys data root (default: ~/.genesys/data)")
    ...
    if args.extract:
        ...
        try:
            from genesys.extraction.live import run_once  # noqa: PLC0415 — lazy, live-only
            ...
            print(f"save_moment: SAVED and queued, but extraction failed ({exc}); "
                  f"run `genesys-worker once` to retry.")


if __name__ == "__main__":  # `python -m genesys.save_moment` (the /save command's entry)
    main()
