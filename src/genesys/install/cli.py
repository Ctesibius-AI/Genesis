"""`genesys-install` — the one-run installer CLI (D-GCW-10).
...
from genesys.install.installer import STORE_MODES, InstallError, install, uninstall
from genesys.install.platform import PlatformError, detect_platform, select_profile
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="genesys-install", description="Wire Genesys into a workspace.")
    p.add_argument("--workspace", default=str(Path.cwd()), help="Workspace to wire (default: cwd).")
    p.add_argument("--data-root", default=str(Path.home() / ".genesys" / "data"),
                   help="Genesys data root for stores + the ledger.")
    ...
    p.add_argument("--uninstall", action="store_true", help="Remove only Genesys-marked wiring.")
    ...
    if args.uninstall:
        ...
        sys.stdout.write(f"Genesys uninstalled from {workspace}: {actions}\n")
        return 0
