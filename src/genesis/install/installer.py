...
Wires a workspace for Genesis: project-local capture hooks + a `.mcp.json` recall registration +
...
- **Clean uninstall** (AC-I3): removes only Genesis-marked entries.
...
from genesis.hooks.wiring import GENESIS_EVENTS, unwire_hooks, write_hook_wiring

GENESIS_HOOK_COMMAND = "python3 -m genesis.hooks.cli"   # carries the genesis.hooks.cli mark
RECALL_MCP_SERVER = "genesis-recall"
_MCP_COMMAND = "python3"
_MCP_ARGS = ["-m", "genesis.recall.mcp_server"]
_ENV_KEYS = ("GENESIS_DATA", "GENESIS_DATA_ROOT", "GENESIS_DB_PATH", "GENESIS_GROUP_ID")
...
def install(workspace: Path, *, data_root: Path, api_key_present: bool,
    ...
    env = genesis_env(data_root, db_path, group_id)

    # 1) capture hooks — PROJECT-LOCAL ONLY (AC-ISO2), merge-never-clobber, consent-gated.
    hook_actions = write_hook_wiring(settings, command=GENESIS_HOOK_COMMAND, consent=True)
...
def uninstall(workspace: Path) -> dict:
    """Remove only Genesis-marked wiring (AC-I3); foreign hooks / servers / env keys survive."""
