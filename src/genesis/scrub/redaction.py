...
Mechanism: the bytes are physically removed and replaced with a tombstone record:
    ...
    {redacted: true, at: <date>, reason, hash: HMAC(local-Genesis-key, removed-content),
     actor}
...
(``genesis.config.get_local_hmac_key``) and is never hardcoded.
...
from genesis.config import get_local_hmac_key
