"""Claude Code hook adapter — dispatch hook events to the Genesis capture pipeline.
...
Spec references:
  ...
  - F-GENESIS-03 (provisional summary ruling): summary = last assistant_text, falling
...
NO network. Stdlib only (plus internal genesis imports).
"""
...
from genesis.hooks.translate import cc_transcript_to_events, provisional_summary
...
def dispatch(
    ...
    *,
    ...
    """Dispatch a Claude Code hook event to the appropriate Genesis pipeline action.
    ...
    Args:
        ...
        data_root: Root directory for Genesis data (ledger, episodes, diary).
    ...
    if event == "SessionStart":
        ...
        from genesis.hooks.confirmation import MemoryState, confirmation_line, memory_state
        try:
