"""Translate Claude Code .jsonl transcript records into Genesis capture event dicts.

Spec references:
  - F-GENESIS-03 (provisional summary ruling): the live save-ritual heuristic is
    ...
    provisional choice pending F-GENESIS-03 design.
...
def cc_transcript_to_events(records: list[dict]) -> list[dict]:
    """Map Claude Code transcript .jsonl records to Genesis capture event dicts.

    Genesis event dict keys (matching TranscriptEvent.from_dict expectations):
    ...
    for rec in records:
        ...
        if rtype == "user":
            ...
            # documented Genesis intake shape (see tests/fixtures/sample_transcript.json);
            # mirror_events turns them into task.* projection entries. This is NOT a guess at
            # a Claude Code transcript schema — it forwards records already in Genesis shape.
...
def last_assistant_text(events: list[dict]) -> str:
    ...
    Spec: provisional jot fallback #1 (F-GENESIS-03).
    """
...
def last_user_text(events: list[dict]) -> str:
    ...
    Spec: provisional jot fallback #2 (F-GENESIS-03).
    """
    for ev in reversed(events):
...
def provisional_summary(events: list[dict]) -> str:
    ...
    RULING (F-GENESIS-03, pending design): provisional summary = last visible
    ...
    F-GENESIS-03 save-ritual heuristic design.
    """
