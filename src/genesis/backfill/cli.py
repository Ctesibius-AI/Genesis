"""genesis-backfill — batch backfill injection door (CLI).

Usage:
    genesis-backfill <paths...> [--data DIR] [--dry-run]

Feeds historical Claude Code .jsonl session transcripts through the EXISTING
capture pipeline (genesis.hooks.adapter.dispatch, SessionEnd path) to enqueue them
...
This CLI does NOT drain/extract — that is genesis.extraction.live.run_once, a
separate step.
"""
...
from genesis.backfill.discover import SessionPlan, build_plan
from genesis.hooks.adapter import dispatch
from genesis.ledger.store import read_all

DEFAULT_DATA = Path("~/.genesis/data").expanduser()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Data root (default: ~/.genesis/data).",
    )
    parser.add_argument(
