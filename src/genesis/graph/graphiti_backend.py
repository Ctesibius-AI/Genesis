"""Real graphiti-core / FalkorDB backend for Genesis (spec §4.9a, F-11, DR-35).
...
# But a Genesis episode is a WHOLE SESSION (DR-25) — 13KB–700KB+ — so 10 raw sessions per
...
class GraphitiCoreClient:
    ...
    def add_episode(self, name: str, body: str, ref_ts: str) -> AddEpisodeResults:
        ...
        async def _add() -> AddEpisodeResults:
            ...
            from genesis.graph.ontology import (
    ...
    def search_edges(self, query: str, top_n: int, method: str) -> list[ClientEdge]:
        ...
        from genesis.graph.ontology import ALLOWED_EDGE_TYPES
    ...
    def set_edge_attributes(self, uuid: str, **attrs: object) -> None:
        """Merge custom Genesis attributes into the entity edge properties (spec §4.9a).
...
def _fetch_api_key(env: Any) -> str:  # noqa: ANN401
    ...
    result = subprocess.run(
        ["security", "find-generic-password", "-a", "genesis",
...
def build_graphiti_client(
    *,
    ...
    ``db_path``/``group_id`` resolves from ``GENESIS_DB_PATH``/``GENESIS_GROUP_ID``
    ...
    from genesis.config import get_db_path, get_group_id
    ...
    from genesis.graph.model_tier import STANDARD_MODEL_DEFAULT, build_tiered_anthropic_client
