...
the build if a pinned-Graphiti upgrade stops stamping transaction-time inside the Genesis
...
def real_client(*, db_path: str | None = None, env: Mapping[str, str] | None = None):
    ...
    # imported/initialized so it's honored regardless of how Genesis is launched. Uses
    ...
    from genesis.graph.graphiti_backend import build_graphiti_client  # noqa: PLC0415
...
def assert_transaction_time_in_window(client, *, now) -> None:  # pragma: no cover - integration
    ...
    assert inv, "version drift: no expired edge attributed inside the Genesis commit window"
    for e in inv:
